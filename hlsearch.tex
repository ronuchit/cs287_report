...
%, $\mu(x) = \frac{1}{1+e^{-w^\top x}}$, and $y$ is the binary label taking on the value $1$ when the action is ``optimal'' and $0$ otherwise. 


% , }}$$ where $f((n,m))$ is the feature vector associated with the current action as defined above. To estimate the parameter vector we maximize the log-likelihood of $w$ given the demonstrated trajectories. The maximum likelihood estimation of the parameters $w$

% Given the human-demonstrated sequence of actions we maximize the log-likelihood: $$ $$
